...
onF$trt_mo <- paste(onF$trt,onF$month)
ests_onF <- NULL
for(i in unique(onF$trt_mo)){
  sub <- onF[onF$trt_mo == i, ]
  ests_onF.i <- coef(summary(lmer(body.length..mm. ~ 1 + (1|trap:site), data = sub, )))[1,1:2]
  ests_onF.i <- data.frame(trt_mo = i, t(ests_onF.i))
...
for(i in unique(onM$trt_mo)){
  ...
  ests_onM.i <- coef(summary(lmer(body.length..mm. ~ 1 + (1|trap:site), data = sub, )))[1,1:2]
  ests_onM.i <- data.frame(trt_mo = i, t(ests_onM.i))
